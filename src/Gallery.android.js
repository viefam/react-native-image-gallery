...
import { Pressable, View, ViewPropTypes } from "react-native"
...
export default class Gallery extends PureComponent {
    ...
    UNSAFE_componentWillMount() {
        ...
        this.gestureResponder = createResponder({
            onStartShouldSetResponderCapture: (evt, gestureState) => {
                //fix for android when first touch is in image area cannot scroll list
                if (this.props.onlySwipeImage && !this.shouldScrollViewPager(evt, gestureState)) return false
                return true
            },
            ...
            onResponderSingleTapConfirmed: (evt, gestureState) => {
                !this.props.onlySwipeImage && this.props.onSingleTapConfirmed && this.props.onSingleTapConfirmed(this.currentPage)
            },
        })
    ...
    render() {
        ...
        const { onSingleTapConfirmed, onlySwipeImage } = this.props
        //fix for android when first touch is in image area cannot scroll list
        return (
            <View
                style={{ flex: 1 }}
            ...
            >
                <Pressable
                    disabled={!onlySwipeImage}
                    onStartShouldSetResponderCapture={() => !!onlySwipeImage}
                    onPress={() => onSingleTapConfirmed && onSingleTapConfirmed(this.currentPage)}
                    style={{ flex: 1 }}>
                    <ViewPager
                        {...this.props}
